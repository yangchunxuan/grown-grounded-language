...
\hypersetup{hidelinks,
  pdftitle={Five Walls on the Path to Growing a Grounded Language: An Adversarial Self-Audit of Emergent Communication},
...
\title{Five Walls on the Path to Growing a Grounded Language: An Adversarial Self-Audit of Emergent Communication}
\author{
    Chunxuan Yang \\
    \mbox{}\\
...
shortcut the world allowed, suggesting that \emph{in our tested regimes world
design was a recurring bottleneck}, and that structure does not grow in worlds too
small to require it. We then
...
we detect \emph{no} co-evolution of the channel itself from random initialization
under survival selection (an apparent intelligibility rise did not separate from a
communication-blind control; $n{=}6$, underpowered). We offer the self-audit protocol, the mapped wall
system, and reproducible verdict artifacts as the contribution.
\end{abstract}

% Choose one of: Full Paper, Summaries, or Late Breaking Abstracts
Submission type: \textbf{Late Breaking Abstracts}\\

Code: \href{https://github.com/yangchunxuan/grown-grounded-language/tree/v1.0-alife2026}{\texttt{github.com/yangchunxuan/grown-grounded-language}}, tag \texttt{v1.0-alife2026}\\
...
JSON. The \emph{architecture-matched null} is the same architecture and parameter
count with the mechanism ablated (an untrained random reservoir, a frozen-random
channel); the \emph{red-team} is a panel of independent agents each constructing a
confound that explains the win without the mechanism, and a surviving confound
forces an added control or a retraction. The rig caught
...
in our tested regimes world design was a recurring bottleneck; and structure does
not grow in worlds too small to require it---\emph{installed} structure works. Each shortcut was first taken seriously and
...
\caption{Results inside the constructed world; values from committed verdict
artifacts, bars are bootstrap 95\% CIs. \textbf{(a)} Lethal signaling (24 seeds):
\emph{fusion} (real content, 0.58) survives far above \emph{scramble} (random,
0.08), \emph{misroute} (correct content, broken content-to-location binding, 0.04)
and \emph{mute} (0.04); \emph{oracle} (fenced) $=1.00$. fusion beats each control
by an exact paired McNemar test ($p<5\mathrm{e}{-}4$); the absolute-majority gate
is inconclusive, so this is \emph{relative}, and \emph{misroute} shows the
content-to-location \emph{binding} (not signal presence) carries survival.
\textbf{(b)} A heritable ``trust'' gene (8 seeds) rises to ${\approx}0.96$ under
real content vs.\ ${\sim}0.3$--$0.4$ under scrambled / no selection: \emph{use} of a
separately-trained channel is selected for (adoption, not de-novo emergence).
\textbf{(c)} Co-evolving from random weights (6 seeds): the apparent
intelligibility rise (0.124) does \emph{not} separate from a communication-blind
control (0.096; survival$-$control CI $[-0.05, 0.11]$ includes $0$; underpowered),
both far above the frozen null (0.002).}
\label{fig:rtc}
\end{figure*}
...
\textbf{Does the channel co-evolve?---no detectable effect, underpowered.} From
random speaker/listener weights under survival selection alone (no truth gradient),
even the strongest legitimate mechanism (tied encode/decode weights plus kin
assimilation) shows no detectable growth of a load-bearing channel
(\Cref{fig:rtc}c): the apparent intelligibility rise did not separate from a
communication-blind control ($n{=}6$, underpowered), consistent with clonal
descent-convergence rather than selection for communication.
...
test) with scoped positives and a co-evolution null (no detectable effect,
underpowered). We suggest
emergent-communication claims be reported against architecture-matched nulls and
communication-blind controls as standard. All code and verdict JSONs are committed
and reproducible from fixed seeds; headline scripts were rerun from a fresh clone
on a separate machine to the same verdicts, with an oracle-leakage audit. AI agents
produced candidate implementations and adversarial critiques under this protocol.
