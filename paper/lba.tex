...
\midrule
Self-supervised prediction & temporal integration by \emph{any} recurrent net (world too dead to have learnable dynamics) & untrained random reservoir beats the trained world-model \\
Within-life optimization & exhaustive coverage / installed structure (space small enough to tabulate) & coverage cliff under a fixed budget \\
Compositional generalization & holistic codes (task too small; symmetric pressures only stall) & held-out joint stays $0$ across eight attacks \\
Cooperation beyond communication & mutual-decode equivalence / basin lottery (population too small) & selection-gate CI crosses $0$ \\
Spatial communication & co-location / flocking (speaker info $\supseteq$ listener need) & content- vs.\ flocking-control comparison \\
\bottomrule
\end{tabular}
\end{table*}
...
artifacts; bars show bootstrap 95\% CIs. \textbf{(a)} Lethal signaling (24 seeds):
agents acting on real decoded content (\emph{fusion}, 0.58) survive far above the
degraded controls---\emph{scramble} (random message, 0.08), \emph{misroute}
(correct content, broken content-to-location binding, 0.04) and \emph{mute} (no
message, 0.04); \emph{oracle} (true argmax, fenced reference) =1.00. fusion beats
every control by an exact paired McNemar test ($p<5\mathrm{e}{-}4$; the run's own
absolute-majority-survival gate is inconclusive, so this is a \emph{relative}
result). The decisive \emph{misroute} control isolates that the content-to-location
\emph{binding}, not mere signal presence, carries survival. \textbf{(b)} A
heritable ``trust'' gene gating use of the channel (8 seeds) rises to
${\approx}0.96$ under real content but stays at the mutation floor
(${\sim}0.3$--$0.4$) under scrambled content or no selection: use of a (separately
trained) channel is \emph{selected for}---adoption, not de-novo emergence.
\textbf{(c)} Co-evolution from random weights under survival selection (6 seeds):
an apparent intelligibility rise (0.124) does \emph{not} separate cleanly from a
communication-blind random-fitness control (0.096; survival$-$control bootstrap CI
$[-0.05, 0.11]$ includes $0$), and both sit far above the frozen null (0.002)---the
rise is clonal descent-convergence, not selection for communication. The
signaling-bootstrap deadlock holds.}
\label{fig:rtc}
\end{figure*}
...
headline results were independently reproduced on a separate machine by a
different model and audited for oracle-leakage. AI agents generated implementation
candidates and adversarial critiques, but all headline verdicts are
