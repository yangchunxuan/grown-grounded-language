...
\item \textbf{g1f (survival-selected channel; 28 generations):} predates the
  ...
  $n{=}48$ RNG-fixed random-fitness control:
  ...
  \texttt{rtc\_g1f\_commblind\_verdict\_formal48\_rngfix.json} @5a6885a (its embedded
  provenance records base commit \texttt{254f3d7} with \texttt{git\_dirty}; it was banked at
  \texttt{5a6885a}). This rngfix verdict is the \textbf{sole source} for every headline
  number. (2) The legacy helpers (\texttt{..\_formal48.json}, \texttt{..\_reconciled\_verdict.json})
  predate the RNG-fix and are \textbf{superseded}. (3) Lineage composition
  (\texttt{RTC\_G1F\_LINEAGE\_SEEDS=48 python -m offscreen.rtc\_g1f\_lineage\_share}):
  \texttt{rtc\_g1f\_lineage\_share\_verdict.json} @5a6885a: dominant-share 99.1\%, N$_{\rm eff}$ 1.02.
