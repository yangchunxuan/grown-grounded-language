...
Code \& data: all source, the hand-coded world, the experiment runners, and every banked verdict JSON are openly available at \url{https://github.com/yangchunxuan/grown-grounded-language}; every result regenerates via the per-stage commands in the Appendix and \texttt{offscreen/RUNBOOK.md}. An archival snapshot is deposited at Zenodo, DOI \url{https://doi.org/10.5281/zenodo.21074235} (concept DOI, always resolving to the latest version).\\
...
scan (\texttt{\_redline\_scan}) enforces a subset on the core co-evolution runner
\texttt{rtc\_g1f\_coevolve.py} only; the cross-lineage runners (C2X/C2X2/C2X3) are enforced
by construction and checked by code-grounded review, not by this automated scan.
...
load-bearing)}: open-channel populations survive far better than muted or scrambled ones
(fraction alive 0.31 vs.\ 0.03 vs.\ 0.05, CIs excluding zero), so message content is
load-bearing for survival. The floor-level CF is therefore genuine cross-lineage
...
Every result is anchored to three durable, reachable handles: (i) the \textbf{verdict JSON},
identified by filename and present both in the repository and in the archived
\texttt{v2.0-g1f} release (Zenodo DOI \url{https://doi.org/10.5281/zenodo.21074235}); (ii) the
claim ledger (\texttt{offscreen/CLAIM\_LEDGER.md}), mapping each claim to its verdict file; and
(iii) the runbook (\texttt{offscreen/RUNBOOK.md}), giving the exact command, env vars, and
effective config per stage. The public git history was subsequently normalized to a single
author, so historical commit hashes are not reachable in the current repository; verify each
result by its verdict filename in the repository, the \texttt{v2.0-g1f} release, or the Zenodo
deposit, and regenerate via the runbook. (Each verdict JSON's embedded
\texttt{provenance.git\_commit} likewise refers to the pre-normalization history.)
...
\begin{itemize}
\item \textbf{g1f (survival-selected channel; 28 generations):} reproduce as a documented
  multi-step sequence. (1) Run the $n{=}48$ RNG-fixed random-fitness control:
  \texttt{RTC\_G1F\_FORMAL=1 RTC\_TOXIC\_DEATH=-0.9 RTC\_G1F\_COMMBLIND\_SEEDS=48 python -m
  offscreen.rtc\_g1f\_commblind\_control}, banked as
  \texttt{rtc\_g1f\_commblind\_verdict\_formal48\_rngfix.json}. This rngfix verdict is the
  \textbf{sole source} for every headline number. (2) The legacy helpers
  (\texttt{..\_formal48.json}, \texttt{..\_reconciled\_verdict.json}) predate the RNG-fix and
  are \textbf{superseded}. (3) Lineage composition
  (\texttt{RTC\_G1F\_LINEAGE\_SEEDS=48 python -m offscreen.rtc\_g1f\_lineage\_share}):
...
\texttt{rtc\_g1f\_coevolve.py}) returns zero hits. This automated scan covers the core
co-evolution runner only; the cross-lineage runners (C2X/C2X2/C2X3) are enforced by
construction and checked by code-grounded review, not by this scan.
